...
public class ThreeRaxStimCombatConcussivePush extends SimpleBuildOrder {
    ...
    public ThreeRaxStimCombatConcussivePush() {
        super(Build.terran()
                ...
                .then().expand()
                .atSupply(20).useAbility(BARRACKS, Abilities.BUILD_REACTOR)
                ...
                .atSupply(32).useAbility(BARRACKS, Abilities.BUILD_TECHLAB)
                .then().useAbility(BARRACKS, Abilities.BUILD_TECHLAB)
                ...
                .atSupply(51).startRepeatingStructure(Abilities.BUILD_SUPPLY_DEPOT).at(PlacementRules.borderOfBase())
                ...
                .then().useAbility(EBAY, Abilities.RESEARCH_TERRAN_INFANTRY_WEAPONS)
                .then().expand()
                .then().attack()
                .then().buildStructure(Abilities.BUILD_REFINERY).at(PlacementRules.freeVespeneGeyser())
                .then().buildStructure(Abilities.BUILD_REFINERY).at(PlacementRules.freeVespeneGeyser())
                .then().buildStructure(Abilities.BUILD_STARPORT).near(Units.TERRAN_FACTORY)
                .then().useAbility(FACTORY, Abilities.BUILD_REACTOR)
                .then().attack()
                ...
                .then().useAbility(BARRACKS, Abilities.BUILD_REACTOR)
                .then().useAbility(FACTORY, Abilities.BUILD_REACTOR)
                .then().startRepeatingUnit(BARRACKS, Abilities.TRAIN_MARINE)
                .then().startRepeatingUnit(BARRACKS, Abilities.TRAIN_MARINE)
                .then().useAbility(BARRACKS, Abilities.BUILD_TECHLAB)
