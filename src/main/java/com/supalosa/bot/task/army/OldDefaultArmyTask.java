...
public abstract class OldDefaultArmyTask extends DefaultTaskWithUnits implements ArmyTask {
    ...
    private final DefaultArmyTaskBehaviour<?,?,?,?> behaviour;

    public OldDefaultArmyTask(String armyName, int basePriority, ThreatCalculator threatCalculator, DefaultArmyTaskBehaviour<?,?,?,?> behaviour) {
        super(basePriority);
    ...
    public void onStep(TaskManager taskManager, AgentWithData agentWithData) {
        ...
        if (gameLoop > nextArmyLogicUpdateAt) {
            nextArmyLogicUpdateAt = gameLoop + armyLogicUpdate(agentWithData, agentWithData, armyPositions);
        }
